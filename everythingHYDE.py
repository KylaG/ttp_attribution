"""
MITRE ATT&CK Threat Actor Attribution System with HyDE (Hypothetical Document Embeddings)

This system analyzes cyber threat reports and attributes them to known threat actors
based on the MITRE ATT&CK framework. It uses an enhanced version of semantic search
called HyDE (Hypothetical Document Embeddings) to improve attribution accuracy.

The HyDE approach:
1. For each MITRE technique, generates hypothetical threat report paragraphs using GPT-4
2. Creates embeddings that blend the original technique descriptions with these hypotheticals
3. This bridges the gap between formal technique descriptions and real-world reporting language

The overall system:
1. Processes MITRE ATT&CK technique descriptions into HyDE-enhanced vector embeddings
2. Extracts techniques mentioned in threat reports using semantic search with higher confidence filtering
3. Builds a probabilistic model based on known threat actor behavior
4. Incorporates prior probabilities of threat actors to improve attribution
5. Evaluates performance through k-fold cross-validation

This approach provides more accurate attribution than standard semantic search by
better understanding the varied ways techniques might be described in real threat reports.
It helps security analysts identify potential threat actors behind new attacks with
greater confidence.
...
def gpt_request(dict):
    """
    Generates hypothetical threat report paragraphs for a MITRE technique using GPT-4.
    
    This is a key component of HyDE (Hypothetical Document Embeddings). For each MITRE
    technique, this function creates several hypothetical paragraphs describing how the
    technique might be used in a real cyber attack. These hypothetical paragraphs are then
    used to enhance the embeddings.
    
    Args:
        dict (dict): Dictionary containing MITRE technique information ('ID', 'Name', 'Description')
    
    Returns:
        list: List of generated hypothetical threat report paragraphs
    """
    try:
        prompts = [
            f"Given the description of the TTP that I have provided to you, write a paragraph about the TTP provided as if it were a tactic used in a larger cyber-attack and mimic how the TTP would be written about as a part of an attack in a threat report. TTP:{dict['ID']} {dict['Name']} \n Description: {dict['Description']}",
            f"Act as if this TTP was just used in a cyber-attack. Write a single paragraph about how TTP was used within the larger campaign, focusing on technical execution of the TTP. \n TTP: {dict['ID']} {dict['Name']} \n Description: {dict['Description']}",
            f"Act like you are a cyber forensic analyst. Write a brief incident response paragraph about how this TTP was integrated into a recent cyber-attack by state-sponsored actors. \n TTP: {dict['ID']} {dict['Name']} \n Description: {dict['Description']}",
            f"Imagine a cyberattack happened. In a single paragraph, explain the technical details of how the threat actor used this tactic to conduct a widespread cyber-attack. \n TTP: {dict['ID']} {dict['Name']} \n Description: {dict['Description']}",
            f"Provide a detailed analysis in a single paragraph, describing how a threat actor implemented the following TTP during a significant cyber-attack. Emphasize the impact and the execution methods used. \n TTP ID: {dict['ID']}, TTP Name: {dict['Name']} \n TTP Description: {dict['Description']}"
        ]
        responses = []
        for prompt in prompts:
            hypotheticals = client.chat.completions.create(
                model="gpt-4",
                messages=[
            ...
            #print(hypotheticals)
            if hypotheticals.choices:
                for choice in hypotheticals.choices:
                    # Accessing content properly depending on the API's choice object structure
        ...
        time.sleep(5)
        print("retrying gpt request")
        return gpt_request(dict)

def create_embeddings(df, multiple=False):
    """
    Creates vector embeddings for MITRE ATT&CK technique descriptions using HyDE approach.
    
    In HyDE mode (multiple=True), this function:
    1. For each MITRE technique, generates hypothetical threat report paragraphs using GPT-4
    2. Creates embeddings for both the original description and the hypothetical paragraphs
    3. Averages these embeddings to create a more robust representation
    
    Without HyDE (multiple=False), it simply embeds the original descriptions.
    
    Args:
        df (DataFrame): DataFrame containing MITRE technique information
        multiple (bool, optional): Whether to use HyDE approach. Defaults to False.
    ...
    ttp_desc = []
    for dict in tqdm(list_of_dicts): # one TTP
        if multiple:
            # create hypothetical document embeddings
            responses = gpt_request(dict)

            #extracted_responses = [choice['message']['content'] for choice in hypotheticals.choices]
            temp_embeddings = []

            for response in responses:
                ttp_desc.append({"ttp": dict['ID'], "ttp_description": dict['Name'], "gpt_text": response})
                #hypothetical_blurb = response.text.strip()
                #print(hypothetical_blurb)
    ...
    ttp_desc = pd.DataFrame(ttp_desc)
    ttp_desc.to_csv("HYDE_text.csv")
    print("Created")

    # Convert to a NumPy matrix
    matrix = np.array(embeddings)

    # Save the matrix into a .npz filex
    np.savez("pruned_hyde_embeddings.npz", matrix=matrix)

    return matrix

def load_embeddings_matrix():
    """
    Loads the pre-computed HyDE embeddings matrix from a saved .npz file.
    
    This function is used to avoid recalculating embeddings when the program
    is run multiple times, improving efficiency.
    
    Returns:
        numpy.ndarray: Matrix of HyDE embeddings for the technique descriptions
    """
    # Load the .npz file
    loaded_data = np.load('pruned_hyde_embeddings.npz')
...
def search(embeddings_matrix, reference_df, query_str, topk=5):
    ...
    semantically similar techniques. It filters results to only include techniques
    with a similarity score >= 0.65.
    ...
    Returns:
        DataFrame: Filtered top k most similar techniques to the query with scores >= 0.65
    """
    response = client.embeddings.create(
            input=query_str,
            model="text-embedding-3-large"
    ...
    filtered_indices = [index for index in indices if scores[index] >= 0.65]

    #return reference_df.iloc[indices]
    return reference_df.iloc[filtered_indices]
...
def get_term_counts(txt_file, embeddings_matrix, reference_df, normalize=False):
    ...
    Includes retry logic for API rate limiting or connection issues.
    ...
    for text_batch in text_batches:
        try:
            relevant_mitre_terms_df = search(embeddings_matrix=embeddings_matrix, reference_df=reference_df, query_str=text_batch, topk=2)
        except:
            sleep(5)
...
def process_split(splits, idx, reference_df, embeddings_matrix):
    """
    ...
    Args:
        ...
        reference_df (DataFrame): DataFrame containing MITRE technique information
        embeddings_matrix (numpy.ndarray): Matrix of HyDE embeddings for all techniques
    ...
    #     with open("train_threat_actor.json", "w") as outfile: 
    #         json.dump(train_threat_actor_term_counts, outfile)
    #     die() 
...
def calculate_probabilities(actual_threat_actor, file, mitre_df, embeddings_matrix, p_mitre_code_given_threat_actor, threat_actor_probabilities, is_test=False, validation_split_num=None):
    """
    ...
    This version incorporates prior probabilities of threat actors to improve attribution.
    ...
    Args:
        ...
        is_test (bool, optional): Whether this is a test file (vs validation). Defaults to False.
        validation_split_num (int, optional): Validation split index. Defaults to None.
    ...
    # do a write out of test term probs
    if not is_test:
        # write out as its own csv inside a directory indicating that it is a validation split
        # go inside right directory using validation_split_number
        # save test_term_probabilities 
        # {filename}_counts.csv
        pass
    else:
        # go into the test directory inside experiment name
        # {filename}_counts.csv
        pass
    ...
    for threat_actor in p_mitre_code_given_threat_actor:
        ...
        # additional part of multiplication added here, Monday, May 6th
        # [TRIAL 1] multiplication: will amplify those that are more likely to be it, will minimize those unlikely!
        #threat_actor_scores[threat_actor] = score 
        threat_actor_scores[threat_actor] = score * threat_actor_probabilities[threat_actor]
...
def validation(train_result, mitre_df, embeddings_matrix, threat_actor_probabilities, test=False, validation_split_num=None):
    """
    ...
    This version incorporates prior threat actor probabilities in the attribution process.
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
        ...
        for actual_threat_actor in split_to_use.keys():
            for file in valid[actual_threat_actor]:
                ...
                future = executor.submit(calculate_probabilities, actual_threat_actor, file, mitre_df, embeddings_matrix, p_threat_actor_given_mitre_code, threat_actor_probabilities, is_test=test, validation_split_num=validation_split_num)
                futures.append(future)
...
if __name__ == "__main__":
    """
    Main execution flow of the threat actor attribution system with HyDE.
    
    This block:
    1. Loads MITRE ATT&CK data
    2. Loads or creates HyDE embeddings
    3. Sets up k-fold cross-validation
    4. Processes each fold to build and evaluate attribution models
    5. Selects the best model based on validation performance
    6. Evaluates the best model on test data
    7. Outputs results and model weights for future use
    """
    ...
    # embeddings_matrix = create_embeddings(mitre_df, True)
    ...
    for threat_actor, threat_actor_splits in generated_splits.items():
        if threat_actor in counts:
            threat_actor_probabilities[threat_actor] = counts[threat_actor] / total_count
        else:
            # If no specific count is provided, default to some value or error handling
            threat_actor_probabilities[threat_actor] = 1 / 29

    results = []
    train_results = []
    print("Going through data splits")
    for k in range(3): #k-fold
        train_result = process_split(generated_splits, k, mitre_df, embeddings_matrix)
        train_results.append(train_result)
        # experiment name
        # create a split-k sub-directory
        # create a train sub-directory
        # save train_result[0] for a k-value
        # create a valid sub-directory
        validation_result = validation(train_result, mitre_df, embeddings_matrix, threat_actor_probabilities, False)
        validation_result.to_csv(f"raw_validation_{k}.csv")
        cleaner = validation_result.groupby('actual_ta')['rank'].mean().reset_index()
        cleaner.to_csv(f"processed_validation_{k}.csv")
    ...
    test_result = validation(train_result=top_model, mitre_df=mitre_df, embeddings_matrix=embeddings_matrix, threat_actor_probabilities = threat_actor_probabilities, test=True)
    print(f"Average test case probability: {test_result}")
    test_result.to_csv(f"single_test_result.csv")